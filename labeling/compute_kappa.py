"""
Finalize eval sets + compute agreement (B3).
...
Single-annotator reality (see the approved plan):
- Trustpilot reports Cohen's kappa between the HUMAN label and the STAR-derived proxy label.
  This is NOT inter-annotator agreement; it is a human-vs-proxy validity check, and with only
  ~5/60 non-negative rows it is a low-support, directional estimate.
- Lowyat has no stars and no second annotator -> kappa = N/A (single-label eval set).
"""
import sys
...
KAPPA_REF = 0.6  # reference threshold only -- NOT an inter-annotator pass/fail here
...
def final_label(row) -> str:
    """final_label if present, else label_1 -- keeps the two-labeler workflow compatible."""
...
def main() -> None:
    ...
    report = ["# Labeling agreement (kappa.md)", "",
              "**Single annotator (Faris).** True inter-annotator Cohen's kappa requires two "
              "independent human labelers (METHODOLOGY.md sec 3.4 / REBUILD_PLAN sec 4); that is "
              "not available here. Trustpilot reports a human-vs-star **proxy** kappa instead, "
              "from a **blind** re-label (the annotator did not see the star rating or any "
              "pre-label, so the agreement is genuine, not anchored) -- a validity check, *not* "
              "inter-annotator agreement. Lowyat has no proxy and no second annotator, so kappa "
              "is N/A.", ""]

"""
Build the two eval-set labeling sheets.
...
Single annotator, so the `_2` columns stay empty.
"""
import sys
...
def make_sheet(pool: pd.DataFrame, pre_label: pd.Series, annotator: str = "annotator",
...
def build_trustpilot(df: pd.DataFrame, blind: bool = False) -> pd.DataFrame:
    ...
    Not a meaningful class balance: the tier holds only ~5 non-negative items. This just
    avoids a 100%-negative pool; proxy kappa here is a low-support, directional estimate.
    """
...
def main() -> None:
    ...
    # Trustpilot: blind slim sheet. The annotator must not see the star rating/pre_label, so
    # the proxy kappa reflects genuine agreement rather than anchoring on the hint.

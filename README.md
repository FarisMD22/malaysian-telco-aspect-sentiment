...
The trained XLM-RoBERTa model (~1.1 GB) is **not** included; it is regenerated by running
the pipeline (`src/finetune_xlmr.py`, GPU required — the Colab notebook in `notebooks/`
provides a GPU path). The baseline, evaluation, and app all run on CPU. Until the
transformer is trained, the web app still opens and the evaluation reports the baseline
results only.
